...
# -*- coding: utf-8 -*-
"""
🔄 REGENERERA RESTERANDE JOBB (UTOM DEPLOJA)
=============================================
✅ 15% anpassning av CV och personligt brev
✅ Inget blandspråk (svenska ELLER engelska)
✅ Ingen jobbeskrivning i personliga brev
✅ Inget företagsnamn/jobbtitel i header
✅ Tar bort gamla filer först
"""
...
def regenerate_job(job_folder: Path, driver, modern_facade, job_number: int):
    ...
    try:
        ...
        print(f"\n🗑️ Raderar gamla PDF:er...")
        deleted_count = 0
        for pdf_file in job_folder.glob("*.pdf"):
            if not pdf_file.name.startswith("TEST_"):  # Behåll TEST-filer från Deploja
                print(f"   Raderar: {pdf_file.name}")
                pdf_file.unlink()
                deleted_count += 1

        if deleted_count > 0:
            print(f"   ✅ {deleted_count} gamla filer raderade")
        else:
            print(f"   ℹ️ Inga gamla filer att radera")
        ...
        # Generera personligt brev (CLEAN, ingen jobbeskrivning, inget företagsnamn)
        print(f"\n💌 Genererar personligt brev (CLEAN, 15% anpassning)...")
...
def main():
    """Main entry point"""
    print("\n" + "="*80)
    print("🔄 REGENERERA RESTERANDE JOBB (UTOM DEPLOJA)")
    print("="*80)
    print("✅ 15% anpassning av CV och personligt brev")
    print("✅ Inget blandspråk (svenska ELLER engelska)")
    print("✅ Ingen jobbeskrivning i personliga brev")
    print("✅ Inget företagsnamn/jobbtitel i header")
    print("="*80)
    ...
    try:
        ...
        # Hitta alla jobbmappar UTOM Job_001 (Deploja)
        all_job_folders = sorted([f for f in output_dir.iterdir() if f.is_dir() and f.name.startswith('Job_')])

        # Skippa Job_001_Deploja
        job_folders = [f for f in all_job_folders if not f.name.startswith('Job_001_Deploja')]

        print(f"\n📂 Hittade {len(all_job_folders)} jobbmappar totalt")
        print(f"⏭️ Skippar Job_001 (Deploja - redan klar)")
        print(f"🔄 Regenererar {len(job_folders)} jobb")
        ...
        print(f"Skippad: Job_001 (Deploja)")
        print(f"📁 Dokument: {output_dir.absolute()}")
        print("="*80)
        print("\n💡 Alla jobb har nu:")
        print("   ✅ Inget blandspråk i CV eller brev")
        print("   ✅ Personliga brev utan jobbeskrivning och företagsnamn")
        print("   ✅ CV och brev med max 15% ändring från original")
